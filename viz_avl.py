...
data = pd.read_csv('time_avl_random.txt', header=None, sep=' ')
...
index = 6
labels = ['insert - O(Nlog(N))', 'contains - O(log(N))', 'remove - O(Nlog(N))', 'insert - O(Nlog(N))', 'contains - O(log(N))', 'remove - O(Nlog(N))']
